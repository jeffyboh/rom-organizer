#!/usr/bin/env python3
"""
Script to populate the systems table with ROM system data.
"""
...
def parse_systems_data(data: str) -> list[tuple[str, str]]:
    """Parse the systems data string into a list of (system, system_name) tuples."""
    systems = []
    for line in data.strip().split('\n'):
        line = line.strip()
        if line and ':' in line:
            system, system_name = line.split(':', 1)
            systems.append((system.strip(), system_name.strip()))
    return systems


def populate_systems():
    """Populate the systems table with the provided data."""
    print("Creating database tables...")
    create_tables()

    print("Parsing systems data...")
    systems = parse_systems_data(SYSTEMS_DATA)
    ...
    try:
        ...
        db.rollback()
        print(f"Error populating systems: {e}")
        raise
    finally:
        db.close()
